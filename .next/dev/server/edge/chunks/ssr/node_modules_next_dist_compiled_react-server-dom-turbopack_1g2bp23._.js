(globalThis["TURBOPACK"] || (globalThis["TURBOPACK"] = [])).push(["chunks/ssr/node_modules_next_dist_compiled_react-server-dom-turbopack_1g2bp23._.js",
 ...
 */ "production" !== ("TURBOPACK compile-time value", "development") && function() {
    ...
    function serializeAsyncIterable(request, task, iterable, iterator) {
        ...
        function error(reason) {
            streamTask.status === PENDING$1 && (request.cacheController.signal.removeEventListener("abort", abortIterable), erroredTask(request, streamTask, reason), enqueueFlush(request), "function" === typeof iterator.throw && iterator.throw(reason).then(noop, noop));
        }
    ...
    function processServerComponentReturnValue(request, task, Component, result) {
        ...
        if (iteratorFn) {
            ...
            $jscomp$compprop0 = ($jscomp$compprop0[Symbol.iterator] = function() {
        ...
        return "function" !== typeof result[ASYNC_ITERATOR] || "function" === typeof ReadableStream && result instanceof ReadableStream ? result : ($jscomp$compprop0 = {}, $jscomp$compprop0 = ($jscomp$compprop0[ASYNC_ITERATOR] = function() {
            ...
            return iterator;
        }, $jscomp$compprop0), $jscomp$compprop0._debugInfo = result._debugInfo, $jscomp$compprop0);
    }
    ...
    function resolveModel(request, task, parent, parentPropertyName, value) {
        var jsonValue = value;
        null !== value && "object" === typeof value && "function" === typeof value.toJSON && (jsonValue = value.toJSON(parentPropertyName));
        var originalValue = parent[parentPropertyName];
        "object" !== typeof originalValue || originalValue === jsonValue || originalValue instanceof Date || callWithDebugContextInDEV(request, task, function() {
            ArrayBuffer.isView(originalValue) ? console.error("Binary data with a toJSON method, such as a Node.js Buffer, is serialized through toJSON instead of as binary. Pass a Uint8Array or ArrayBuffer to send binary data.%s", describeObjectForErrorMessage(parent, parentPropertyName)) : "Object" !== objectName(originalValue) ? "string" === typeof jsxChildrenParents.get(parent) ? console.error("%s objects cannot be rendered as text children. Try formatting it using toString().%s", objectName(originalValue), describeObjectForErrorMessage(parent, parentPropertyName)) : console.error("Only plain objects can be passed to Client Components from Server Components. %s objects are not supported.%s", objectName(originalValue), describeObjectForErrorMessage(parent, parentPropertyName)) : console.error("Only plain objects can be passed to Client Components from Server Components. Objects with toJSON methods are not supported. Convert it manually to a simple value before passing it to props.%s", describeObjectForErrorMessage(parent, parentPropertyName));
        });
        value = renderModel(request, task, parent, parentPropertyName, jsonValue);
        if (null === value || "object" !== typeof value) return value;
        if (isArrayImpl(value)) {
            var _resolved = [];
            for(jsonValue = 0; jsonValue < value.length; jsonValue++)_resolved[jsonValue] = resolveModel(request, task, value, "" + jsonValue, value[jsonValue]);
            return _resolved;
        }
        jsonValue = {};
        for(_resolved in value)if (hasOwnProperty.call(value, _resolved)) {
            var resolvedValue = resolveModel(request, task, value, _resolved, value[_resolved]);
            _resolved === __PROTO__$1 ? Object.defineProperty(jsonValue, _resolved, {
                value: resolvedValue,
                enumerable: !0,
                writable: !0,
                configurable: !0
    ...
    function serializeErrorValue(request, error) {
        ...
        "cause" in error && (message = outlineModel(request, error.cause), name.cause = serializeByValueID(message));
        "undefined" !== typeof AggregateError && error instanceof AggregateError && (error = outlineModel(request, error.errors), name.errors = serializeByValueID(error));
        return "$Z" + outlineModel(request, name).toString(16);
    }
    function emitErrorChunk(request, id, digest, error, debug, owner) {
        var name = "Error", env = (0, request.environmentName)(), causeReference = null, errorsReference = null;
        try {
            if (error instanceof Error) {
                ...
                if ("undefined" !== typeof AggregateError && error instanceof AggregateError) {
                    var errors = error.errors, errorsId = debug ? outlineDebugModel(request, {
                        objectLimit: 5
                    }, errors) : outlineModel(request, errors);
    ...
    function emitTypedArrayChunk(request, id, tag, typedArray, debug) {
        ...
        id = stringToChunk(id);
        debug ? request.completedDebugChunks.push(NEXT_TWO_CHUNKS_ARE_ATOMIC, id, typedArray) : request.completedRegularChunks.push(NEXT_TWO_CHUNKS_ARE_ATOMIC, id, typedArray);
    }
    function emitTextChunk(request, id, text, debug) {
        ...
        id = stringToChunk(id);
        debug ? request.completedDebugChunks.push(NEXT_TWO_CHUNKS_ARE_ATOMIC, id, text) : request.completedRegularChunks.push(NEXT_TWO_CHUNKS_ARE_ATOMIC, id, text);
    }
    function renderDebugModel(request, counter, parent, parentPropertyName, value) {
        ...
        if ("object" === typeof value) {
            ...
            if (value instanceof Error) {
                ...
                "cause" in value && (counter.objectLimit--, entry = outlineDebugModel(request, counter, value.cause), key.cause = serializeByValueID(entry));
                "undefined" !== typeof AggregateError && value instanceof AggregateError && (counter.objectLimit--, value = outlineDebugModel(request, counter, value.errors), key.errors = serializeByValueID(value));
        ...
        if ("string" === typeof value) {
            if (1e6 < value.length) return "This string of length " + value.length + " has been omitted by React to avoid sending too much data from the server.";
    ...
    function flushCompletedChunks(request) {
            ...
            try {
                for(var debugChunks = request.completedDebugChunks, i = 0; i < debugChunks.length; i++){
                    ...
                    if (item === NEXT_TWO_CHUNKS_ARE_ATOMIC) {
                        if (i + 2 >= debugChunks.length) throw Error("A chunk pair is incomplete. This is a bug in React.");
                        request.pendingDebugChunks -= 2;
                        writeChunk(debugDestination, debugChunks[i + 1]);
                        writeChunk(debugDestination, debugChunks[i + 2]);
                        i += 2;
                    } else request.pendingDebugChunks--, writeChunk(debugDestination, item);
                }
            ...
            try {
                ...
                if (null === request.debugDestination) {
                    ...
                    for(debugChunks = 0; debugChunks < _debugChunks.length; debugChunks++){
                        ...
                        if (_item === NEXT_TWO_CHUNKS_ARE_ATOMIC) {
                            if (debugChunks + 2 >= _debugChunks.length) throw Error("A chunk pair is incomplete. This is a bug in React.");
                            request.pendingDebugChunks -= 2;
                            writeChunk(debugDestination, _debugChunks[debugChunks + 1]);
                            importsChunks = writeChunkAndReturn(debugDestination, _debugChunks[debugChunks + 2]);
                            debugChunks += 2;
                        } else request.pendingDebugChunks--, importsChunks = writeChunkAndReturn(debugDestination, _item);
                        if (!importsChunks) {
                            request.destination = null;
                            debugChunks++;
                            break;
                ...
                for(debugChunks = 0; debugChunks < regularChunks.length; debugChunks++){
                    ...
                    if (_item2 === NEXT_TWO_CHUNKS_ARE_ATOMIC) {
                        if (debugChunks + 2 >= regularChunks.length) throw Error("A chunk pair is incomplete. This is a bug in React.");
                        request.pendingChunks -= 2;
                        writeChunk(debugDestination, regularChunks[debugChunks + 1]);
                        _debugChunks = writeChunkAndReturn(debugDestination, regularChunks[debugChunks + 2]);
                        debugChunks += 2;
                    } else request.pendingChunks--, _debugChunks = writeChunkAndReturn(debugDestination, _item2);
                    if (!_debugChunks) {
                        request.destination = null;
                        debugChunks++;
                        break;
        ...
        0 === request.pendingChunks && (regularChunks = request.debugDestination, 0 === request.pendingDebugChunks ? (null !== regularChunks && (regularChunks.close(), request.debugDestination = null), request.status < ABORTING && request.cacheController.abort(Error("This render completed successfully. All cacheSignals are now aborted to allow clean up of any unused resources.")), null !== request.destination && (request.status = CLOSED, request.destination.close(), request.destination = null), null !== request.debugDestination && (request.debugDestination.close(), request.debugDestination = null)) : null !== regularChunks && null !== request.destination && (request.status = CLOSED, request.destination.close(), request.destination = null));
    }
    ...
    function abort(request, reason) {
        if (!(11 < request.status)) try {
            ...
            logRecoverableError(request, error$8, null), fatalError(request, error$8);
        }
    }
...
//# sourceMappingURL=node_modules_next_dist_compiled_react-server-dom-turbopack_1g2bp23._.js.map