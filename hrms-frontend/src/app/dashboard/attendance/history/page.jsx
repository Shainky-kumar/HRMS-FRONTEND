...
//       <div className="mb-6">
//         <h1 className="text-[22px] font-semibold text-[#1a1a1a]">Attendance History</h1>
//         <p className="mt-1 text-sm text-[#6b7280]">View past attendance records</p>
//       </div>

//       {error && <div className="mb-4 rounded-md bg-[#fef2f2] px-4 py-3 text-sm text-[#b91c1c]">{error}</div>}

//       <div className="overflow-hidden rounded-lg border border-[#e5e7eb] bg-white shadow-sm">
//         <div className="flex flex-col gap-3 border-b border-[#e5e7eb] px-5 py-3.5 sm:flex-row sm:flex-wrap sm:items-end">
//           <input value={employeeId} onChange={(e) => setEmployeeId(e.target.value)} placeholder="Employee ID" className="rounded-md border border-[#d1d5db] px-3 py-2 text-sm focus:border-[#E42527] focus:outline-none focus:ring-1 focus:ring-[#E42527]" />
//           <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="rounded-md border border-[#d1d5db] px-3 py-2 text-sm" />
//           <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="rounded-md border border-[#d1d5db] px-3 py-2 text-sm" />
//           <button type="button" onClick={() => { setPage(1); fetchData(); }} className="rounded-md bg-[#E42527] px-4 py-2 text-sm font-medium text-white hover:bg-[#c91f21]">Search</button>
//         </div>
...
//           ) : list.length === 0 ? (
//             <div className="py-16 text-center text-sm text-[#6b7280]">No records found</div>
//           ) : (
...
//                   <th className="px-5 py-3 font-medium text-[#6b7280]">Punch In</th>
//                   <th className="px-5 py-3 font-medium text-[#6b7280]">Punch Out</th>
//                   <th className="px-5 py-3 font-medium text-[#6b7280]">Work (min)</th>
//                   <th className="px-5 py-3 font-medium text-[#6b7280]">Late</th>
//                 </tr>
//               </thead>
//               <tbody className="divide-y divide-[#f3f4f6]">
//                 {list.map((row, i) => (
//                   <tr key={row.attendance_id || i} className="hover:bg-[#fafafa]">
//                     <td className="px-5 py-3.5 whitespace-nowrap">{formatDate(row.attendance_date)}</td>
//                     <td className="px-5 py-3.5">{row.employee_id || "—"}</td>
//                     <td className="px-5 py-3.5">{row.status || "—"}</td>
//                     <td className="px-5 py-3.5 whitespace-nowrap">{formatDateTime(row.first_punch_in)}</td>
//                     <td className="px-5 py-3.5 whitespace-nowrap">{formatDateTime(row.last_punch_out)}</td>
//                     <td className="px-5 py-3.5">{row.total_work_minutes ?? "—"}</td>
//                     <td className="px-5 py-3.5">{row.is_late ? `${row.late_minutes || 0}m` : "—"}</td>
//                   </tr>
//                 ))}
//               </tbody>
//             </table>
...
//               <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="rounded-md border border-[#d1d5db] px-3 py-1.5 text-sm disabled:opacity-40">Previous</button>
//               <button disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="rounded-md border border-[#d1d5db] px-3 py-1.5 text-sm disabled:opacity-40">Next</button>
//             </div>
//           </div>
//         )}
//       </div>
