...
//     setError("");
//     try {
//       const res = await api.get("/api/v1/attendence/special_requests", {
...
//     try {
//       // NOTE: adjust this endpoint to match your actual employee list API.
//       // Common patterns in this codebase: "/api/v1/employee/all",
//       // "/api/v1/employees", "/api/v1/get/employees" — change as needed.
//       const res = await api.get("/api/v1/get/employees")
//       setEmployees(toArray(res?.data));
//     } catch (err) {
//       // Don't block the whole page if this fails — just show empty dropdown
//       // with the underlying error surfaced once.
//       setError((prev) => prev || formatApiError(err));
//       setEmployees([]);
//     } finally {
...
//     setSuccess("");
//     try {
//       await api.post("/api/v1/add/attendence/special_request", {
...
//       setForm({ employee_id: "", request_type: "wfh", start_date: "", end_date: "", reason: "", location_name: "", latitude: "", longitude: "" });
//       fetchList();
//     } catch (err) {
//       setError(formatApiError(err));
//     } finally {
...
//     try {
//       await api.put(`/api/v1/attendence/special_request/${id}/approve`);
//       setSuccess("Approved");
//       fetchList();
//     } catch (err) {
//       setError(formatApiError(err));
...
//     const name = emp.full_name || emp.name || [emp.first_name, emp.last_name].filter(Boolean).join(" ");
//     return name ? `${name} (${emp.employee_id})` : emp.employee_id;
//   };
...
//         <p className="mt-1 text-sm text-[#6b7280]">WFH / On Duty / Outdoor</p>
//       </div>
...
//             className="rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm disabled:opacity-60"
//           >
...
//           <select value={form.request_type} onChange={(e) => setForm((p) => ({ ...p, request_type: e.target.value }))} className="rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm">
//             <option value="wfh">Work From Home</option>
//             <option value="on_duty">On Duty</option>
//             <option value="outdoor">Outdoor</option>
//           </select>
//           <input required type="date" value={form.start_date} onChange={(e) => setForm((p) => ({ ...p, start_date: e.target.value }))} className="rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm" />
//           <input required type="date" value={form.end_date} onChange={(e) => setForm((p) => ({ ...p, end_date: e.target.value }))} className="rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm" />
//           <input placeholder="Location name" value={form.location_name} onChange={(e) => setForm((p) => ({ ...p, location_name: e.target.value }))} className="rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm" />
//           <input placeholder="latitude" value={form.latitude} onChange={(e) => setForm((p) => ({ ...p, latitude: e.target.value }))} className="rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm" />
//           <input placeholder="longitude" value={form.longitude} onChange={(e) => setForm((p) => ({ ...p, longitude: e.target.value }))} className="rounded-md border border-[#d1d5db] px-3 py-2.5 text-sm" />
...
//           <button type="submit" disabled={saving} className="rounded-md bg-[#E42527] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#c91f21] disabled:opacity-60 sm:w-fit">{saving ? "Saving..." : "Submit"}</button>
//         </form>
//       </div>
...
//                     <td className="px-5 py-3.5 uppercase">{r.request_type}</td>
//                     <td className="px-5 py-3.5">{formatDate(r.start_date)}</td>
//                     <td className="px-5 py-3.5">{formatDate(r.end_date)}</td>
//                     <td className="px-5 py-3.5 capitalize">{r.status}</td>
//                     <td className="px-5 py-3.5 capitalize">{r.latitude}</td>
//                     <td className="px-5 py-3.5 capitalize">{r.longitude}</td>
//                     <td className="px-5 py-3.5 text-right">
//                       {r.status === "pending" && (
//                         <button onClick={() => approve(r.request_id)} className="text-xs font-medium text-green-600">Approve</button>
//                       )}
//                     </td>
//                   </tr>
//                 ))}
...
export default function SpecialRequestsPage() {
  ...
  useEffect(() => {
    fetchList();
  }, [page, onlyMyApprovals, statusFilter]);
